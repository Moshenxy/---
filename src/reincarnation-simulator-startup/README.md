...
2.  **激发创意**：由AI（天衍）基于玩家的构想，演绎并生成多套完整的、包含全新自定义选项的开局方案。
...
5.  **潜力与背包 (`PotentialAndBackpackPage.vue` / `BackpackManager.vue`)**:
    *   分配潜力值，并选择或调整初始背包物品。
...
-   **`WorldEvolutionService.ts`**: 新增服务，负责与AI交互，根据玩家输入生成动态的、全新的开局方案。
-   **`PresetService.ts`**: 新增服务，提供了强大的方案管理功能，包括对自定义选项的提取、保存、加载、导入/导出。
-   **`TavernService.ts`**: 其内部的Prompt模板已根据最新的变量规则 (`[InitVar].txt` v6.0+) 进行了全面更新和强化，以确保生成的变量结构正确无误。
-   **`store.ts`**:
    *   `removeCustomItem` 函数已修复，现在支持删除所有类型的自定义项目，包括背包物品。
    *   `selections` 对象已扩展，加入了 `character_traits` 和 `core_need`，以支持更完整的角色设定。
