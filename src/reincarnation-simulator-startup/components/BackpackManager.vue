  ...
  <div class="backpack-manager">
      ...
      <div class="dropdown-wrapper">
        ...
        <div class="custom-options">
          <template v-for="category in itemCategories" :key="category.type">
            ...
            <div class="option-item" v-for="item in availableItems(category.type)" :key="item.id" @click="addItem(item.id)">
              <span>{{ item.name }} ({{ item.cost }}点)</span>
               <button v-if="item.custom" @click.stop="removeCustomItem(category.type, item.id)" class="delete-custom-item">×</button>
            </div>
          </template>
        </div>
      </div>
    ...
    <div class="selections-container">
      <div v-for="(count, itemId) in store.selections.backpack" :key="itemId" class="selected-item">
        <Tooltip :content="getItemDescription(itemId)">
          <span class="item-tag">{{ getItemNameAndCategory(itemId) }} x {{ count }}</span>
        </Tooltip>
...
.item-tag {
    background-color: rgba(10, 150, 150, 0.2);
    color: #82d8d8;
    padding: 6px 12px;
    border-radius: 12px;
    font-size: 0.9em;
...
.item-tag:hover {
    background-color: rgba(10, 150, 150, 0.4);
}
</style>