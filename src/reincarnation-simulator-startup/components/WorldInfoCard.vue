        ...
        <template v-if="epochId === worldData.元规则.当前纪元ID">
            ...
            <div v-if="epoch.内容" class="subsection">
              ...
              <div v-if="epoch.内容.历史?.历史纪元">
                ...
                <div v-for="(era, key) in epoch.内容.历史.历史纪元" :key="key">
                  <p><strong>{{ era.纪元名称 }}:</strong> {{ era.简述 }}</p>
                </div>
              </div>
            </div>
            ...
            <div v-if="epoch.规则" class="subsection">
              ...
              <div v-if="epoch.规则.演化逻辑?.演化规则?.length">
                ...
                <ul><li v-for="(rule, index) in epoch.规则.演化逻辑.演化规则" :key="index">{{ rule.描述 }}</li></ul>
              </div>
            </div>
            ...
            <div v-if="epoch.力量体系" class="subsection">
              ...
              <div v-if="epoch.力量体系.机制核心?.length" class="tag-group">
                <h6>机制核心:</h6>
                <ul><li v-for="(core, index) in epoch.力量体系.机制核心" :key="index">{{ core }}</li></ul>
              </div>
              <div v-if="epoch.力量体系.专长与流派 && Object.keys(epoch.力量体系.专长与流派).filter(k => k !== '$meta').length > 0" class="tag-group">
                <h6>专长与流派:</h6>
                <div class="tags-container">
                  <Tooltip v-for="school in Object.values(epoch.力量体系.专长与流派).filter(item => typeof item === 'object')" :key="school.流派名称" :content="school.核心理念 || ''">
                    <span class="tag">{{ school.流派名称 }}</span>
                  </Tooltip>
                </div>
              </div>
              <div v-if="epoch.力量体系.关键资源依赖 && Object.keys(epoch.力量体系.关键资源依赖).filter(k => k !== '$meta').length > 0" class="tag-group">
                <h6>关键资源依赖:</h6>
                <div class="tags-container">
                  <span v-for="resource in Object.values(epoch.力量体系.关键资源依赖).filter(item => typeof item === 'string')" :key="resource" class="tag tag-resource">{{ resource }}</span>
                </div>
              </div>
              <div v-if="epoch.力量体系.属性模板 && Object.keys(epoch.力量体系.属性模板).filter(k => k !== '$meta').length > 0" class="tag-group">
                <h6>属性模板:</h6>
                <div class="tags-container">
                ...
                <ul class="level-list">
                  <li v-for="level in Object.values(epoch.力量体系.境界定义).filter(item => typeof item === 'object')" :key="level.名称" class="level-item">
                    ...
                    <div v-if="expandedLevels[level.名称]" class="level-details">
                      ...
                      <div v-if="level.解锁系统 && level.解锁系统.名称 !== '无'"><p><strong>解锁系统:</strong> {{ level.解锁系统.名称 }} - {{ level.解锁系统.描述 }}</p></div>
                      <div v-if="level.晋升需求 && level.晋升需求.条件"><p><strong>晋升需求:</strong> {{ level.晋升需求.条件.描述 }}</p></div>
                    </div>
                  </li>
                </ul>
              </div>
