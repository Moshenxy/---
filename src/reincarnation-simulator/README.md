...
## 近期重要更新 (V18.0 - AI自我修正与闭环学习)

1.  **【核心·交互】引入“天道回溯”机制**:
    *   **闭环学习**: 重构了 `actions.ts` 中的核心交互逻辑，建立了一个数据反馈闭环。现在，在每次向AI（天衍）发送新指令前，系统会自动提取上一轮AI响应中的`<JSONPatch>`变量变更内容。
    *   **上下文附加**: 这个`<JSONPatch>`块会被包裹在一个新的`<上轮变量变更>`标签内，并附加到当前用户指令的最前端，一同发送给AI。
    *   **AI自我修正**: 此机制为AI提供了“审查”其上一步操作的机会。通过阅读上一轮的变量更新，AI能够在进行新一轮推演时，发现并修正潜在的逻辑错误或数据不一致，完美实现了`【cot】天衍之心.txt`中“第零阶段：天道回溯 · 稽查旧账”的设计理念。
