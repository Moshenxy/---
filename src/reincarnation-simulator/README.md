...
1.  **【核心交互】详情弹窗内联化**:
    *   **弹窗组件升级**: 重构了点击正文中的NPC名称时的交互逻辑。现在，弹出的详情窗口将直接渲染“众生名录”中使用的、功能更完整的 `NpcDetailDisplay.vue` 组件，取代了之前简陋的 `NpcInfoCard.vue`。
    *   **UI/UX优化**: 为详情弹窗设置了固定的宽度和高度，并启用了内容区域的内部滚动。这解决了之前因内容切换（如在“档案”、“权能”、“因果”标签页之间切换）导致弹窗尺寸“跳动”的问题，提供了更稳定、更流畅的视觉体验。

## 近期重要更新 (V18.0 - AI思考链重构与数据自检闭环)

1.  **【核心·AI思考】引入“逻辑内核+人格化外壳”COT模型**:
    *   **流程重构**: 彻底重构了 `【cot】天衍之心.txt`，将原有的九阶线性流程，优化为更精简、更高效的“七步执行清单”。
    *   **人格注入**: 在纯逻辑思考链的末尾，新增了【天衍锐评】阶段。AI将在完成所有推演后，以“天衍”的第一人称视角，对整个事件进行一次充满其性格色彩的“复盘”与“点评”，实现了逻辑严谨性与人格化魅力的完美结合。

2.  **【核心·数据流】实现“天道回溯”数据自检闭环**:
    *   **静默缓存**: 前端现在会自动、静默地读取AI上一次响应中的`<JSONPatch>`内容并进行缓存。
    *   **因果链输入**: 在下一次向AI发送请求时，会将缓存的`JSONPatch`作为`<previous_patch>`，与玩家的当前输入一同发送。
    *   **自我稽查**: 新的COT【步骤1：数据自检】将强制AI优先审查这个`<previous_patch>`，检查自己上一轮的变量更新是否存在逻辑错误或结构缺陷（如缺少`$meta`），并生成补救指令。这构成了一个完美的、即时的自我纠错闭环，极大地增强了系统的长期运行稳定性。
