...
const age = computed(() => {
  if (!props.npc.出生日期 || !store.worldState?.世界) return '未知';

  const npcWorldId = Array.isArray(props.npc.所属世界) ? props.npc.所属世界[0] : props.npc.所属世界;
  const world = store.worldState.世界[npcWorldId];
  if (!world) return '未知';

  const currentEpochId = world.元规则?.当前纪元ID;
  if (!currentEpochId) return '未知';

  const currentEpoch = world.历史纪元?.[currentEpochId];
  const currentTime = currentEpoch?.当前时间;
  if (!currentTime) return '未知';

  const currentYear = currentTime.年;
  const birthYear = props.npc.出生日期.年;
...
const getMoodDescription = (emotions: { 心绪: number; 怒意: number; 胆识: number; 仪态: number; }): string => {
  ...
  const emotionMapping: { [key in EmotionKey]: { [range: number]: string } } = {
    心绪: { 100: '极乐', 90: '心花怒放', 80: '狂喜', 70: '振奋', 60: '兴奋', 50: '愉悦', 40: '欣喜', 30: '高兴', 20: '愉快', 10: '开心', 0: '平静', '-10': '低落', '-20': '郁闷', '-30': '失落', '-40': '沮丧', '-50': '悲伤', '-60': '痛苦', '-70': '心碎', '-80': '悲痛欲绝', '-90': '万念俱灰', '-100': '绝望' },
    怒意: { 100: '憎恨', 90: '雷霆之怒', 80: '暴怒', 70: '大怒', 60: '怒火中烧', 50: '愤怒', 40: '烦躁', 30: '恼火', 20: '不满', 10: '微愠', 0: '中立', '-10': '平静', '-20': '耐心', '-30': '淡然', '-40': '平和', '-50': '镇定', '-60': '冷静', '-70': '沉着', '-80': '波澜不惊', '-90': '心如止水', '-100': '绝对理智' },
    胆识: { 100: '英勇无双', 90: '悍不畏死', 80: '奋勇', 70: '果敢', 60: '无畏', 50: '勇敢', 40: '从容', 30: '沉着', 20: '安定', 10: '镇定', 0: '平静', '-10': '警惕', '-20': '紧张', '-30': '担忧', '-40': '不安', '-50': '畏惧', '-60': '害怕', '-70': '恐惧', '-80': '惊恐万状', '-90': '魂飞魄散', '-100': '崩溃' },
    仪态: { 100: '傲视群雄', 90: '意气风发', 80: '神采飞扬', 70: '挥洒自如', 60: '自信', 50: '从容', 40: '大方', 30: '放松', 20: '自在', 10: '自然', 0: '平静', '-10': '拘谨', '-20': '不自在', '-30': '局促', '-40': '腼腆', '-50': '害羞', '-60': '难为情', '-70': '窘迫', '-80': '羞愧难当', '-90': '极度羞耻', '-100': '无地自容' }
  };
