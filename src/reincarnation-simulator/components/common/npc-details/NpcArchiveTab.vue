...
import { calculateAge, mainWorld, avatarWorld } from '../../../store/getters';
...
const age = computed(() => {
  const world = computed(() => {
    if (!store.worldState?.世界 || !props.npc.所属世界) return null;
    return store.worldState.世界[props.npc.所属世界] || null;
  });
