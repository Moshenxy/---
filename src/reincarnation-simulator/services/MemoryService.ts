...
class MemoryService {
  ...
  public isAutoMemoryEnabled = true;
   ...
   * 更新所有层级的记忆
   */
  public async updateMemory(worldLog: WorldLogEntry[], force = false): Promise<void> {
    if (!this.isAutoMemoryEnabled && !force) {
      console.log('[MemoryService] Auto memory update is disabled. Skipping.');
      return;
    }
    console.log(`[MemoryService] Updating all memory tiers... (Forced: ${force})`);
    try {
      // 改为串行调用以避免竞态条件
  ...
  public setAutoMemoryEnabled(enabled: boolean) {
    this.isAutoMemoryEnabled = enabled;
    this.saveSettings();
  }
  ...
  private saveSettings() {
    try {
      const settings = {
        config: this.config,
        isAutoMemoryEnabled: this.isAutoMemoryEnabled,
      };
      localStorage.setItem('reincarnation_memory_service_settings', JSON.stringify(settings));
    } catch (error) {
      console.error('[MemoryService] Failed to save settings to localStorage:', error);
    }
  }

  private loadSettings() {
    try {
      const savedSettings = localStorage.getItem('reincarnation_memory_service_settings');
      if (savedSettings) {
        const settings = JSON.parse(savedSettings);
        this.config = settings.config || this.config;
        this.isAutoMemoryEnabled = settings.isAutoMemoryEnabled ?? this.isAutoMemoryEnabled;
      }
    } catch (error) {
      console.error('[MemoryService] Failed to load settings from localStorage:', error);
    }
  }
}
