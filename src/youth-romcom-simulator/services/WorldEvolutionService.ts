...
class WorldEvolutionService {
    ...
    for (const world of Object.values(worldState.世界)) {
        ...
        for (const charId in characters) {
            ...
            if (isAmbitious || isPowerful) {
                const firstGoal = isAmbitious ? (Object.values(desire)[0] as any) : { 名称: '维持自身存在', 动机: '其强大的力量无时无刻不在影响周围。' };
   ...
   */
  public simulateReactions(pressures: EvolutionaryPressure[], worldState: AppState['worldState']): StressReactionEvent[] {
    ...
    for (const pressure of pressures) {
      if (pressure.intensity > 50) { // 仅当压力足够大时才触发
        const sourceChar = get(worldState, `世界.${entityIndexService.findById(pressure.source.id)?.worldId}.角色.${pressure.source.id}`);
        if(sourceChar) {
  ...
  public propagateRipples(event: StressReactionEvent, worldState: AppState['worldState']) {
    ...
    console.log(`[EvoSys-Ripple] New ripple queued. Distance: ${distance}km. Arrival in ${delayDays.toFixed(1)} days.`, newRipple);
  ...
  public getAvailableRipples(currentTime: number): InformationRipple[] {
    ...
    if (availableRipples.length > 0) {
        console.log(`[EvoSys-Gravity] ${availableRipples.length} ripples have arrived and are now available as 'narrative material'.`, availableRipples);
    }
