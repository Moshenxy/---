  ...
  try {
    ...
    if (entry) {
      ...
      await TavernHelper.createLorebookEntries(LOREBOOK_NAME, [
        { comment: entryName, content, keys: [entryName], enabled: true },
      ]);
    }
 ...
 */
export async function appendToLorebook(
  entryName: string,
  newContent: string,
  separator: string = '\n\n',
): Promise<void> {
  try {
    ...
    if (entry) {
      ...
      await TavernHelper.createLorebookEntries(LOREBOOK_NAME, [
        { comment: entryName, content: newContent, keys: [entryName], enabled: true },
      ]);
    }
 ...
 */
export async function appendToLorebookByIndex(
  entryIndex: number,
  newContent: string,
  separator: string = '\n\n',
  bookName: string,
  ...
  try {
    ...
    if (entry.comment.startsWith('占位条目-') && entryIndex === 18) {
      // 18是第19个条目的索引
