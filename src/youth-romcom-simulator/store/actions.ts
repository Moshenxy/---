...
async function fetchCoreData() {
  try {
    ...
    if (messages && messages.length > 0 && messages[0].data) {
      ...
      const gametxtRegex = /<gametxt>([\s\S]*?)<\/gametxt>/gi;
      const matches = [...(fullMessageContent?.matchAll(gametxtRegex) || [])];

      if (matches.length > 0) {
        const allGametxtContents = matches.map(match => match[1].trim());
        store.mainWorldNarrative = allGametxtContents.join('<hr class="narrative-divider">');
      }
...
async function processAiResponse(response: string, prefixedInput: string) {
  ...
  const gametxtRegex = /<gametxt>([\s\S]*?)<\/gametxt>/gi;
  const matches = [...response.matchAll(gametxtRegex)];

  let narrative = null;
  if (matches.length > 0) {
    const allGametxtContents = matches.map(match => match[1].trim());
    narrative = allGametxtContents.join('<hr class="narrative-divider">');
...
async function handleAction(actionText: string, isReroll = false) {
  ...
  // 关闭“变量列表”世界书条目
  await lorebookService.setEntryEnabled('变量列表', false);
...
${buildXmlTag('地点', worldContextData.地点)}
