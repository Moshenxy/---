...
const 日程时间规则Schema = z.object({
  ...
  时间片段: z.array(z.enum(['清晨', '上午前', '上午', '午休', '下午', '下午后', '夜晚'])).optional(),
});
...
const 每日倾向Schema = z.partialRecord(
  z.enum(['早晨', '上学路', '午前', '午休', '午后', '放学后', '傍晚', '夜']),
  z.array(倾向条目Schema),
);
...
export const RootSchema = z.object({
  世界状态: z
      ...
      时间: z
        .object({
          日期: z
            ...
            .regex(/^\d{4}-\d{2}-\d{2}$/)
            .default('2012-04-08'),
          当前片段: z.enum(['清晨', '上午前', '上午', '午休', '下午', '下午后', '夜晚']).default('清晨'),
          星期: z.enum(['日', '一', '二', '三', '四', '五', '六']).default('日'),
          当日类型: z.enum(['工作日', '周末', '节假日']).readonly().default('工作日'),
        })
        .default({ 日期: '2012-04-08', 当前片段: '清晨', 星期: '日', 当日类型: '工作日' }),
      行动点: z
        .object({
    ...
    .default({
      时间: { 日期: '2012-04-08', 当前片段: '清晨', 星期: '日', 当日类型: '工作日' },
      行动点: { 当前: 2, 上限: 2 },
      状态: '日常的世界',
      天气: '晴天',
      未来天气: '晴天',
  ...
  命运卡牌系统: z
    .object({
      当日卡牌预告: z
        .object({
          类型: z.string().nullable().default(null),
          等级: z.string().nullable().default(null),
        })
        .default({ 类型: null, 等级: null }),
      最后掷骰日期: z
        .string()
        .regex(/^\d{4}-\d{2}-\d{2}$/)
        .default('2000-01-01'),
      当日卡牌已确认: z.boolean().default(false),
    })
    .default({
      当日卡牌预告: { 类型: null, 等级: null },
      最后掷骰日期: '2000-01-01',
      当日卡牌已确认: false,
    }),
  叙事记录: z
